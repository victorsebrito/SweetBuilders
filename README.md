...
1. `GenericBuilder<T>.Default` works just like calling `Fixture.Build<T>()` without any customizations
```c#
var foo = GenericBuilder<Foo>.Default
  .With(x => x.Prop1, "abc")
  .Create();
...
2. `GenericBuilder<T>.Empty` uses the default constructor of `T` (even if it's private), and does not generate values automatically
```c#
var foo = GenericBuilder<Foo>.Empty
  .With(x => x.Prop1)
  .Create();
...
3. `GenericBuilder<T>.Uninitialized` creates an instance of `T` without using its constructors, and does not generate values automatically
```c#
var foo = GenericBuilder<Foo>.Uninitialized
  .With(x => x.Prop1, "abc")
  .Create();
